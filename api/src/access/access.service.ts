...
import { ACCESS_GRANT_CMD, DOOR_TRIGGER_OPEN_CMD, ACCESS_GET_USER_ACCESSES_IN_BUILDING_CMD } from '@siam/common';
...
export class AccessService {
  ...
  getUserAccessesInBuilding(userId: number, buildingId: number) {
    this.logger.log(`Getting access events (RPC) for User ${userId} in Building ${buildingId}`, 'AccessService');
    
    return this.client.send({ cmd: ACCESS_GET_USER_ACCESSES_IN_BUILDING_CMD }, { userId, buildingId })
      .pipe(
        timeout(this.config.get<number>(RMQ_MESSAGE_TIMEOUT_TOKEN, 5000)),
        catchError(error => {
          if (error.status) {
          ...
          return throwError(() => new InternalServerErrorException(error.message || 'Error getting user accesses'));
        })
      );
  }
}